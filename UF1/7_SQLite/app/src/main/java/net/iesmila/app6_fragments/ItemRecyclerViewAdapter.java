...
public class ItemRecyclerViewAdapter extends RecyclerView.Adapter<ItemRecyclerViewAdapter.ViewHolder> {
    ...
    public ItemRecyclerViewAdapter(List<Item> items, OnListFragmentInteractionListener listener , String selectedItemId) {
        mValues = items;
        mListener = listener;
    ...
    public void setSelectedItemId(String selectedItemId) {
        for(int i=0;i<mValues.size();i++) {
            if(mValues.get(i).getId().equals(selectedItemId)){
                int mPreviousSelectedPosition = mSelectedPosition;
                mSelectedPosition = i;
                notifyItemChanged(mSelectedPosition);
                if(mPreviousSelectedPosition>=0) notifyItemChanged(mPreviousSelectedPosition);
                break;
            }
        }
    }
